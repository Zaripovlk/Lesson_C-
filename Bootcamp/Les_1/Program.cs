/* Виды алгоритмов 
1. Константные O(1) - O(2)
2. Логарифмические
3. Линейные O(n) - O(2*n)...
4. Линейно-логарифмические
5. Квадратные
6. Кубические
*/
/* Напишите программу, которая считает сумму чисел от 1 до n.(пункт 1,3)
Console.Clear();
Console.Write ("Введите число: ");
int n = int.Parse(Console.ReadLine()!), result=0; 
// Console.ReadLine принимает значение строки.
// "!" убирает предупреждение пустого ввода.
//int.Parse работает быстрее за счет того что сохраняет преоб число в ту же ячейку памяти.
//int n = Convert.ToInt32(Console.ReadLine()); 
// Работает дольше так как функция считывает и 
//записывает преобразованное число в другую ячейку памяти.
for (int i=1; i <= n; i++)
    result += i;
Console.WriteLine($"Сумма чисел от 1 до {n} равна {result} ");
*/
Console.Clear();
Console.Write("Введите число: ");
double n = int.Parse(Console.ReadLine()!);
Console.WriteLine($"Сумма числа от 1 до {n} равна {(1 + n) / 2.0* n}");

//Бинарный поиск (двоичный поиск)
// Загадонное число равно 67
// от 1 до 100
// Число больше 50? - Да (от 50 до 100)
// Число больше 75? - нет (от 50 до 75)
// Число больше 62? - да (62 до 75)
// Число больше 68? - нет (от 62 до 68)
// Число больше 65? - да (от 65 до 68)
//число больше 66? - да (от 66 до 68)
//число больше 67? - нет (ответ: 67)
// 7 попыток
// Сложность алгоритма бинарного поиска равен О(Log2(n))
// log2(1000) ~ 10/
// [34, 1, 2, -10, 56, 314, 13, 1 , 2, 4, 90, -123, 32, 54, 652]
// O(n * log(n)) + O(log2(n))

// [34, -10, 23, 5, 2, 1]
// 1. Выбирается опорный элемент(в основном берется первый элемент массива)
// 2. Создается 2 массива. 1-ый массив содержит элементы меньше опорного,
// 2-ой массив содержит элементы болише или равные опорному.

// Опорный элемент равен 34 [-10, 1, 2, 5, 23, 34]
// [-10, 23, 5, 2, 1] + [34] + []
// Опорный элемент -10 [-10, 1, 2, 5, 23]
// [] + [-10] + [23, 5, 2, 1]
// Опорный элемент 23 [1, 2, 5, 23]
// [5, 2, 1] + [23] + []
// Опорный элемент 5 [1, 2, 5]
// [2, 1] + [5] + []
// Опороный элемент 2
// [1] + [2] + []